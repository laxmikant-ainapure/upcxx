...
#define SET_GEX_OP_PROTOTYPE(T) \
      template<> \
      void call_gex_AD_OpNB(uintptr_t ad, T* p, upcxx::global_ptr<T>, atomic_op opcode, \
  ...
  class atomic_domain {
    ...
    public:
      ...
      template<typename Cxs = FUTURE_CX>
      FETCH_RTYPE<T, Cxs> load(global_ptr<T> gptr,
                               std::memory_order order = std::memory_order_relaxed,
                               Cxs cxs = FUTURE_CX{{}}) {
      ...
      template<typename Cxs = FUTURE_CX>
      FETCH_RTYPE<T, Cxs> compare_exchange(global_ptr<T> gptr, T val1, T val2,
                                           std::memory_order order = std::memory_order_relaxed,
                                           Cxs cxs = FUTURE_CX{{}}) {
